...
@Component("smsApi")
public class SmsApiSend { 
	
	private static Logger logger = LoggerFactory.getLogger(SmsApiSend.class);
	...
	public void sendSms() {
		...
		try {
			...
			File dir = new File(smsApiUpload);
					...
					if(requestFile.equals(requestFile2)) {
						...
						if(index == endFile) {
							...
							for(int p=0;p<lastFile.size();p++) {
								...
								if(new File(smsApiUpload+"/"+lastFile.get(p)).exists()) {
								
									jsonNode = objectMapper.readTree(new File(smsApiUpload+"/"+lastFile.get(p)));
								    String target = ((String) lastFile.get(p)).substring(0, ((String) lastFile.get(p)).lastIndexOf('.')) + ".ING";
								    
								    File baseFile = new File(smsApiUpload+"/"+lastFile.get(p));
								    File newFile = new File(smsApiUpload+"/"+target);
								    File sucessFile = new File(smsApiUpload+"/bak/"+lastFile.get(p)+"_"+currentDate());
									...
									smsApiService.insertNeoSms(smsApiVO);
									}
							}
						}
					}
...
//				logger.debug("Sms Api Send Target Not Exist");
//				return;
//			}
//			
//			
...
//			}
		...
		int result =0;
					...
					for (int i= 0; i < smsApiTargetList.size(); i ++) {
						...
						if(segmentVO != null && !"".equals(segmentVO.getCreateTy())) {
							...
							if (segmentMemberVO != null) {
								//senderTel = EncryptUtil.getJasyptDecryptedString(algorithm, keystring, smsApiTargetList.get(i).getSendTelno());
								//senderTel = smsApiTargetList.get(i).getSendTelno();
								//smsApiTargetList.get(i).setSendTelno(senderTel);
								result = insertMessageQueue(smsApiTargetList.get(i),segmentVO, segmentMemberVO, currentDate());
							}
						} else {
	...
	private List<SmsApiVO> getTargetList(String sendDate){
		...
		try {
			smsTargetList = smsApiService.getSmsListTarget(smsApiVO);
			if(smsTargetList!=null && smsTargetList.size() > 0 ) {
				...
				try {
					for (int i= 0; i < smsTargetList.size(); i ++) {
						...
						//smsTargetList.get(i).setSendTelno(smsTargetList.get(i).getSendTelno());
					}
				} catch(Exception e) {
					logger.error("smsService.updateSmsStatus error = " + e);
				}
			} else {
				//logger.debug("SMS Send Target Data is Nothing");
			}
		} catch(Exception e) {
	...
	private void setTargetStatus( List<SmsApiVO> smsTargetList,  String status){
		try {
			...
			if(smsTargetList!=null && smsTargetList.size() > 0 ) {
				...
				logger.debug("SMS Set Target Status Data is Nothing");
			}
		} catch(Exception e) {
	...
	private SegmentMemberVO ReadSegmentQuery(SegmentVO segmentVO){
		...
		try {
			...
			logger.debug("smsService.getDbConnInfo error = " + e);
		}
	...
	private List<SmsQueueVO> insertQueue(SmsApiVO smsApiVO,SmsQueueVO smsQueueVO, String sendDate, List<SmsQueueVO> listReceiver) {
		...
		smsQueueVO.setSendPhone(smsApiVO.getSendTelno());
