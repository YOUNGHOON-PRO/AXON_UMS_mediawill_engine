...
@Component("neoEmailApi")
public class NeoEmailApiSend { 
	
	private static Logger logger = LoggerFactory.getLogger(NeoEmailApiSend.class);
	...
	private NeoEmailApiService neoEmailApiService;
	...
	public void sendEmail() {
		...
		neoEmailApiService =  (NeoEmailApiService) BeanUtils.getBean("neoEmailApiService");
		...
		try {
			...
			File dir = new File(neoEmailApiUpload);
					...
					if(requestFile.equals(requestFile2)) {
						...
						if(index == endFile) {
							...
							for(int p=0;p<lastFile.size();p++) {
								...
								if(new File(neoEmailApiUpload+"/"+lastFile.get(p)).exists()) {
								
									jsonNode = objectMapper.readTree(new File(neoEmailApiUpload+"/"+lastFile.get(p)));
								    String target = ((String) lastFile.get(p)).substring(0, ((String) lastFile.get(p)).lastIndexOf('.')) + ".ING";
								    
								    File baseFile = new File(neoEmailApiUpload+"/"+lastFile.get(p));
								    File newFile = new File(neoEmailApiUpload+"/"+target);
								    File sucessFile = new File(neoEmailApiUpload+"/bak/"+lastFile.get(p)+"_"+currentDate());
									...
									if( jsonList != null){
										for(int z=0;z< jsonList.size(); z++ ){
											...
											neoEmailApiService.insertNeoEmailApiStg(emailApiList);
											emailApiList.clear();											
										}
			...
			logger.error("neoEmailApiService.sendEmail error = " + e);
